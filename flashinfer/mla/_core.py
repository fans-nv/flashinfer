...
def trtllm_batch_decode_sparse_mla_dsv4(
    ...
    out_scales : Optional[torch.Tensor]
        ...
        ``[num_heads // 8, sum_q, 8, 512]``. Requires all 128 query heads on the
        calling rank (DP attention, no attention TP); ask
        :func:`dsv4_fused_epilogue_available` first and on ``False`` use the BF16
        output and quantize it yourself. Columns past ``sum_q`` are left untouched.

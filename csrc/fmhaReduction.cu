...
bool hasDsv4FusedEpilogueReduction(TllmGenFmhaKernelMetaInfo const& kernelMeta) {
...
void runFmhaReduction(TllmGenFmhaKernelMetaInfo const& kernelMeta, KernelParams const& params,
  ...
  FLASHINFER_CHECK(!dsv4FusedEpilogue || hasDsv4FusedEpilogueReduction(kernelMeta),
                   "No DSv4 fused-epilogue reduction kernel for this descriptor");
  ...
  if (dsv4FusedEpilogue) {
    FLASHINFER_CHECK(numCtasForAllHeads * numHeadsPerCta == params.mNumHeadsQ,
                     "DSv4 fused-epilogue reduction needs headsQ to tile the head CTAs exactly");

...
#pragma unroll
    ...
    // DSv4 FP8 output: quantize the merged FP32 values, as the fused cubin does.
    if constexpr (Dsv4Fp8Out) {
...
static int32_t reductionHeadDimPerCtaV(TllmGenFmhaKernelMetaInfo const& kernelMeta) {
...
bool hasDsv4Fp8ReductionEpilogue(TllmGenFmhaKernelMetaInfo const& kernelMeta) {
  int32_t const headDimPerCtaV = reductionHeadDimPerCtaV(kernelMeta);
  // mGroupsHeadsQ makes the softmax-stats row a flat [token, headQ] index; the rest are the
  // specializations instantiated in runFmhaReduction.
  return isGmemReductionWithSeparateKernel(
         ...
         kernelMeta.mGroupsHeadsQ && kernelMeta.mTileSizeQ == 64 && kernelMeta.mTileSizeKv == 128 &&
         (headDimPerCtaV == 128 || headDimPerCtaV == 256 || headDimPerCtaV == 512) &&
         kernelMeta.mHeadDimQk == flashinfer::dsv4::kHeadDim &&
...
void runFmhaReduction(TllmGenFmhaKernelMetaInfo const& kernelMeta, KernelParams const& params,
  ...
  bool const dsv4Fp8Output = params.ptrDsv4OScaleFp32 != nullptr;
  FLASHINFER_CHECK(!dsv4Fp8Output || hasDsv4Fp8ReductionEpilogue(kernelMeta),
                   "No DSv4 FP8-output reduction epilogue for this kernel");
  ...
  void (*kernel)(KernelParams const, bool, bool, bool, int32_t, int32_t, int32_t, int32_t) =
      nullptr;
  if (dsv4Fp8Output) {
    FLASHINFER_CHECK(numCtasForAllHeads * numHeadsPerCta == params.mNumHeadsQ,
                     "DSv4 FP8-output reduction needs headsQ to tile the head CTAs exactly");
